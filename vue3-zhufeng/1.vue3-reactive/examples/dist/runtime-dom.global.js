var VueRuntimeDOM = (function (exports) {
  ...
  const FORM_TAGS = 'button,datalist,fieldset,input,keygen,label,legend,meter,optgroup,option,' +
      'output,progress,select,textarea';
  const isHTMLTag = /*#__PURE__*/ makeMap(HTML_TAGS);
  const isSVGTag = /*#__PURE__*/ makeMap(SVG_TAGS);
  const isFormTag = /*#__PURE__*/ makeMap(FORM_TAGS, true);
  ...
  const effectStack = []; // 创建effect栈 包装effect执行顺序正确
  ...
  function createGetter(isReadonly = false, shallow = false) {
      return function get(target, key, receiver) {
          ...
          else if (key === "__v_raw" /* RAW */ && // 可以使用toRaw方法获取被代理过对象对应的原值
  ...
  function createSetter(shallow = false) {
      return function set(target, key, value, receiver) {
          ...
          if (target === toRaw(receiver)) {
              // ??????????? 原型链
              if (!hadKey) {
                  trigger(target, "add" /* ADD */, key, value); // 新增
              }
  ...
  function deleteProperty(target, key) {
      ...
      const oldValue = target[key]; // 以前也有值
  ...
  function set$1(key, value) {
      ...
      if (!hadKey) {
          trigger(target, "add" /* ADD */, key, value); // 触发更新 -》 添加逻辑
      }
      else if (hasChanged(value, oldValue)) {
          trigger(target, "set" /* SET */, key, value, oldValue); // 触发修改
      }
      return this;
  }
  ...
  function reactive(target) {
      ...
      return createReactiveObject(target, // (arr obj)  (map set)
      false, mutableHandlers, // new Proxy 对应的handler
  ...
  function createReactiveObject(target, isReadonly, baseHandlers, collectionHandlers) {
      ...
      if (target["__v_raw" /* RAW */] && // readonly(reactive(obj))
      ...
      target, targetType === 2 /* COLLECTION */ ? collectionHandlers : baseHandlers);
  ...
  class RefImpl {
      ...
      get value() {
          track(toRaw(this), "get" /* GET */, 'value'); // ref 可能被reactive包裹
          return this._value;
      }
      set value(newVal) {
  ...
  function renderComponentRoot(instance) {
      ...
      try {
          ...
          if (vnode.shapeFlag & 4 /* STATEFUL_COMPONENT */) {
              ...
              result = normalizeVNode(render.call(// 获取render的返回值
              proxyToUse, proxyToUse, renderCache, props, setupState, data, ctx));
  ...
  */
  ...
  function createRenderer(options) {
      return baseCreateRenderer(options); // 创建渲染器
  }
  ...
  function baseCreateRenderer(options, createHydrationFns) {
          ...
          switch (type) {
              ...
              default:
                  if (shapeFlag & 1 /* ELEMENT */) {
                      ...
                      type.process(// 处理teleport
                      n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, optimized, internals);
                  }
                  else if (shapeFlag & 128 /* SUSPENSE */) {
                      type.process(// 处理suspense
                      n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, optimized, internals);
                  }
                  else {
      ...
      const mountElement = (vnode, container, anchor, parentComponent, parentSuspense, isSVG, optimized) => {
          ...
          const { type, props, shapeFlag, transition, scopeId, patchFlag, dirs } = vnode;
          {
              ...
              vnode.type, isSVG, props && props.is);
              // mount children first, since some props may rely on child content
              // being already rendered, e.g. `<select value>`
              if (shapeFlag & 8 /* TEXT_CHILDREN */) { // 儿子是文本直接插入
                  hostSetElementText(el, vnode.children);
              }
              else if (shapeFlag & 16 /* ARRAY_CHILDREN */) {
                  mountChildren(// 儿子是数组 循环挂载
                  vnode.children, el, null, parentComponent, parentSuspense, isSVG && type !== 'foreignObject', optimized || !!vnode.dynamicChildren);
              }
              if (dirs) {
                  invokeDirectiveHook(vnode, null, parentComponent, 'created');
              }
      ...
      const patchElement = (n1, n2, parentComponent, parentSuspense, isSVG, optimized) => {
          ...
          if (patchFlag > 0) {
              ...
              if (patchFlag & 16 /* FULL_PROPS */) { // 动态属性 全量diff
      ...
      n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, optimized) => {
          if (n1 == null) {
              if (n2.shapeFlag & 512 /* COMPONENT_KEPT_ALIVE */) { // keep-alive流程
      ...
      const patchChildren = (n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, optimized = false) => {
          ...
          if (patchFlag > 0) {
              if (patchFlag & 128 /* KEYED_FRAGMENT */) {
                  ...
                  patchKeyedChildren(//
                  c1, c2, container, anchor, parentComponent, parentSuspense, isSVG, optimized);
                  return;
              }
          ...
          if (shapeFlag & 8 /* TEXT_CHILDREN */) { // 新文本 老数组
              ...
              if (prevShapeFlag & 16 /* ARRAY_CHILDREN */) { // 新数组  老数组
                  ...
                  if (prevShapeFlag & 8 /* TEXT_CHILDREN */) { // 老文本 新数组
  ...
  function _createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null, // 存储动态属性
  isBlockNode = false) {
